...
통신 방식(TCP/Serial)에 종속되지 않도록 `IModbusTransport` 인터페이스를 구현하여 다형성을 확보했습니다. 이를 통해 `ModbusClient`는 하위 통신 방식의 변화에 영향을 받지 않습니다.
...
3. **Application Layer**: 예외를 직접 던지지 않고 `ModbusResult` 객체로 래핑하여 UI 레이어의 부담을 최소화.
